<script>
    ...
    let pageTitle = `Watch ${data.info.video.data.title} Episode ${data.info.video.data.episode}${data.info.video.data.is_sub ? " English Subbed" : ""}`;
</script>
...
<div class="px-2">
    ...
    <div class="aspect-video my-2">
        {#if !data.info.video.data.hls}
            <iframe src={data.info.video.data.iframe_url} title={pageTitle}  frameborder="0"></iframe>
        {:else}
            <Stream url_hls={data.info.video.data.hls}/>
        {/if}
    </div>
    ...
    <div class="text-center flex gap-2 py-2">
        <Lazy height={360}>
            <img src={op_img(data.info.cards.data.episode[0].image.url)} onerror="this.src='{op_img(data.info.cards.data.episode[0].image.alt_url ?? false)}';"  alt="{data.info.video.data.title} Episode {data.info.video.data.episode}" class="w-full max-h-[360px] object-cover rounded" >
        </Lazy>
        <p class="w-[80%]">{data.info.video.data.description}</p>
    </div>
    {#if user && watched}
    ...
    <div class="episode">
        {#each data.info.cards.data.episode as eps}
            <a data-sveltekit-reload href={eps.url.split('/')[2]} class="hover:text-green-500 hover:font-bold hover:text-md">
                <div class="flex gap-2 py-1 justify-between border-b-2">
                    <p class="truncate">{eps.title}</p>
